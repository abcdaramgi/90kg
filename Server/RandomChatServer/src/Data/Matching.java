...
public class Matching implements Runnable{
    ArrayList<Client> availableClients = new ArrayList<Client>();
...
//        }
    ...
    private void randomMatching() {
        availableClients = SocketManager.getInstance().matchingClients;
        ...
        if(sourceClient != targetClient){
            ...
            SocketManager.getInstance().matchingClients.remove(SocketManager.getInstance().matchingClients.indexOf(sourceClient));
...
//                // 조건 만족 시 프리미엄 배열에 추가
//                if (((ageFilter >= min) && (ageFilter <= max)) && (condition[3].equals(genderFilter))) {
//
